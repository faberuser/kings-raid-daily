...
def config():
    ...
    stockage = con(
        "Farm random rewards (fragments/books) in stockage (make sure already set up all team in all dungeons)"
    )
                ...
                if devices != "":
                    ...
                    for num in devices.split():
                        if num.isnumeric():
                            ...
                            print(
                                "Invalid index in emulators config (index must be an interger), please try again"
                            )
                            continue
                    while True:
                        max_devices = input(
                        ...
                        if max_devices.isnumeric() == False:
                            print(
                                "Value must be an interger (>=1/greater or equal to 1), please try again"
                            )
                            continue
    ...
    while True:
        ...
        if bonus_cutoff.isnumeric() == False:
            print(
                "Value must be an interger (>=0/greater or equal to 0), please try again"
            )
            continue
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            print(
                f"2) Run this script in background to check and run when new day (at {tm})"
            )
            print(
                f"3) Make this script auto start in background upon Windows startup to check and run when new day (at {tm})"
            )
            ...
            if auto_daily.isnumeric() == False:
                ...
                if int(auto_daily) == 1:
                    ...
                    clear()
                    print(
                        f"Ok, this scripts will run in background to check and run the script when new day (at {tm}) (please don't close this window)"
                    )
                    while True:
                    ...
                    clear()
                    startup = pth.expanduser(
                        "~\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Startup"
                    )
                    parent = getcwd()[:-16]
                    copy(parent + "kings-raid-daily-background.lnk", startup)
                    print(
                        f'Copied shortcut "kings-raid-daily-background" to "{startup}", script will now auto run in background upon Windows startup\n(Restart Windows to take effect)\n'
                    )
