...
def crop(img, dimesions):
    ...
    # cropped image
    im = img.crop(
        (dimesions[0], dimesions[1], width - dimesions[2], height - dimesions[3])
    )
    return im
...
class Missions:
        ...
        while True:
            try:
                ...
                device.pull(
                    ...
                    "./.cache/screencap-" + str(device.serial) + ".png",
                )
                ...
                if check_crash == True:
                    if self.cache_2 is not None:
                        try:
                            if (
                                check_similar(
                                    self.cache_2, im, 5, self.gb_cf["bonus_cutoff"]
                                )
                                == "similar"
                            ):
                                self.game_count += 1
                                if self.game_count >= 50:  # game freeze
                                    device.shell(
                                        "am force-stop com.vespainteractive.KingsRaid"
                                    )
                                    slp(3)
                                    device.shell(
                                        "monkey -p com.vespainteractive.KingsRaid 1"
                                    )
                                    slp(30)
                                    self.game_count = 0
    ...
    def check_login(self, device, ck_special_shop):
        ...
        while True:
            try:
                ...
                # if android home screen, chrome or play store is opening
                current_window = device.shell(
                ...
                if "com.android.launcher" in str(current_window):
                    ...
                    slp(30)
                elif "com.android.chrome" in str(current_window):
                    slp(30)
                ...
                def update_apk_check():
                    ...
                    if update_apk == "similar":
                        if self.gb_cf["update_apk"] == True:
                            text = (
                                device.serial
                                + ": New APK required to launch, downloading new APK from official site and installing..."
                            )
                            logging.info(text)
                            print(text)
                        else:
                            text = (
                                device.serial
                                + ": New APK required to launch but 'update_apk' set to False, terminated"
                            )
                            logging.info(text)
                            print(text)
                            exit()
                        ...
                        soup = BeautifulSoup(cnt, "html.parser").find_all(
                        ...
                        if pth.exists("./" + name) == True:
                            ...
                            for file in listdir("./"):
                                if file.endswith(".apk"):
                                    remove("./" + file)
                            urlretrieve(
                                "https://client-app.kingsraid.com/apk/" + name,
                                "./" + name,
                            )
                        slp(3)
                        run_(
    ...
    def run_execute(self, device, launched=None):
        ...
        try:
            ...
            if launched is not None:
                text = (
                    device.serial
                    + ": connection to remote host was forcibly closed, closing emulator"
                )
                print(text)
                logging.warn(text)
    ...
    def execute(self, device, launched=None):
        ...
        if self.gb_cf["buff"] == True:
            ...
            self.make_sure_loaded(
                ...
                data["buff"]["1"]["dms"],
                data["buff"]["1"]["shell"],
                second_img="./base/other/etc_2.png",
                third_img="./base/other/etc_3.png",
                cutoff=8,
            )
            # claim exp buff
            self.make_sure_loaded(
                "./base/other/use_exp.png",
                device,
                data["buff"]["2"]["dms"],
                data["buff"]["2"]["shell"],
                cutoff=15,
        ...
        text = (
            device.serial
            + ": opened and claimed rewards (and exp/gold buff) on daily mission board after launch game"
        )
        logging.info(text)
        print(text)
        ...
        if lang is None:
            text = (
                device.serial
                + ": language not supported or cannot recognized (supported languages: english, japanese, vietnamese)"
            )
            logging.info(text)
            print(text)
            if self.launched is not None:
                text = (
                    device.serial
                    + ": because launched from config so closing after done"
                )
                logging.info(text)
                print(text)
        ...
        while True:
            ...
            visible_missions = [
                crop(im, data["first mission"]),
                crop(im, data["second mission"]),
                crop(im, data["third mission"]),
                crop(im, data["fourth mission"]),
            ]
                ...
                if count >= 20:
                    ...
                    if self.gb_cf["loh"] == True:
                        ...
                        if re != "success":
                            text = (
                                device.serial
                                + ": loh not enough currency or unavailable"
                            )
                            logging.info(text)
                            print(text)
                    text = (
                        device.serial
                        + ": all avalible missions has been completed, script ended"
                    )
                    logging.info(text)
                    print(text)
                    if self.launched is not None:
            ...
            for mission in visible_missions:
                ...
                re = self.do_mission(
                    text, device, data["shortcut"][str(count_)], data, size_, lang
                )
                if re == "not":
    ...
    def dragon(self, device, position, data, lang):
        ...
        shortcut = self.make_sure_loaded(
            "./base/dragon/raid_list.png",
            device,
            data["dragon"]["1"]["dms"],
            data["dragon"]["1"]["shell"] + position,
            cutoff=20,
            loop=20,
            sleep_duration=10,
        )
        ...
        # slot 4
        self.make_sure_loaded(
            "./base/dragon/party_1.png",
            device,
            data["dragon"]["10"]["dms"],
            data["dragon"]["10"]["shell"],
            oposite=True,
            sleep_duration=1,
        )
        # slot 5
        self.make_sure_loaded(
            "./base/dragon/party_6.png",
            device,
            data["dragon"]["11"]["dms"],
            data["dragon"]["11"]["shell"],
            oposite=True,
            sleep_duration=1,
        )
        # slot 6
        self.make_sure_loaded(
            "./base/dragon/party_5.png",
            device,
            data["dragon"]["12"]["dms"],
            data["dragon"]["12"]["shell"],
            oposite=True,
            sleep_duration=1,
        )
        ...
        # click start battle
        self.make_sure_loaded(
            "./base/dragon/battle.png",
            device,
            data["dragon"]["13"]["dms"],
            data["dragon"]["13"]["shell"],
            cutoff=30,
        )
        ...
        # click exit
        self.make_sure_loaded(
            "./base/other/my_info.png",
            device,
            data["my_info"]["dms"],
            data["dragon"]["16"]["shell"],
            sleep_duration=0.5,
            cutoff=15,
        )
    ...
    def friendship(self, device, position, data):
        ...
        # click mission shortcut
        shortcut = self.make_sure_loaded(
            "./base/friendship/friends.png",
            device,
            data["friendship"]["1"]["dms"],
            data["friendship"]["1"]["shell"] + position,
            loop=20,
            cutoff=20,
            sleep_duration=10,
        )
        ...
        self.make_sure_loaded(
            "./base/friendship/exchange.png",
            device,
            data["friendship"]["2"]["dms"],
            data["friendship"]["2"]["shell"],
            cutoff=10,
            shell_first=True,
            loop=30,
        )
    ...
    def inn(self, device, position, data):
        ...
        # click mission shortcut
        shortcut = self.make_sure_loaded(
            "./base/inn/visit_inn.png",
            device,
            data["inn"]["1"]["dms"],
            data["inn"]["1"]["shell"] + position,
            cutoff=20,
            loop=20,
            sleep_duration=10,
        )
        ...
        def gift():
            ...
            self.make_sure_loaded(
                ...
                data["inn"]["3"]["dms"],
                data["inn"]["3"]["shell"],
                second_shell=data["inn"]["2"]["shell"],
                cutoff=10,
                second_img="./base/inn/greet_.png",
                third_img="./base/inn/greet__.png",
                loop=5,
                shell_first=True,
            )
            self.make_sure_loaded(
                "./base/inn/start_conversation.png",
                device,
                data["inn"]["4"]["dms"],
                data["inn"]["4"]["shell"],
                second_shell=data["inn"]["2"]["shell"],
                cutoff=10,
                second_img="./base/inn/start_conversation_.png",
                third_img="./base/inn/start_conversation__.png",
                loop=5,
                shell_first=True,
            )
            self.make_sure_loaded(
                "./base/inn/send_gift.png",
                device,
                data["inn"]["5"]["dms"],
                data["inn"]["5"]["shell"],
                second_shell=data["inn"]["2"]["shell"],
                cutoff=10,
                second_img="./base/inn/send_gift_.png",
                third_img="./base/inn/send_gift__.png",
                loop=5,
                shell_first=True,
            )
        ...
        def choose_hero(tap1, tap2):
            self.make_sure_loaded(
                ...
                device,
                data["inn"]["6"]["dms"],
                data["inn"]["6"]["shell"] + str(tap1) + " " + str(tap2),
                shell_first=True,
                second_img="./base/inn/inn_.png",
                cutoff=25,
                second_shell=data["inn"]["2"]["shell"],
    ...
    def lov(self, device, position, data):
        ...
        self.make_sure_loaded(
            ...
            sleep_duration=0.5,
            cutoff=25,
            second_shell=data["lov"]["6"]["second_shell"],
        )
    ...
    def shop(self, device, position, data):
        ...
        # click mission shortcut
        shortcut = self.make_sure_loaded(
            "./base/shop/use_shop.png",
            device,
            data["shop"]["1"]["dms"],
            data["shop"]["1"]["shell"] + position,
            loop=20,
            cutoff=20,
            sleep_duration=10,
        )
        ...
        self.make_sure_loaded(
            ...
            device,
            data["shop"]["3-2"]["dms"],
            data["shop"]["3-2"]["shell"] + str(r[0]) + " " + str(r[1]),
            cutoff=1,
        )
        ...
        device.shell(data["my_info"]["shell"])
        logging.info(
            device.serial + ": successfully bought stuffs in shop in inn mission"
        )
        self.shop_ = True
        return "success"
    ...
    def stockage(self, device, position, data):
        ...
        # click mission shortcut
        shortcut = self.make_sure_loaded(
            "./base/stockage/enter_dungeons.png",
            device,
            data["stockage"]["1"]["dms"],
            data["stockage"]["1"]["shell"] + position,
            loop=20,
            cutoff=20,
            sleep_duration=10,
        )
        ...
        # open stockage
        self.make_sure_loaded(
            "./base/stockage/stockage.png",
            device,
            data["stockage"]["2"]["dms"],
            data["stockage"]["2"]["shell"],
            cutoff=9,
            sleep_duration=0.5,
        )
        ...
        # fragment dungeons
        self.make_sure_loaded(
            "./base/stockage/fragment_d.png",
            device,
            data["stockage"]["3"]["dms"],
            data["stockage"]["3"]["shell"],
            sleep_duration=0.5,
        )
        logging.info(device.serial + ": clicked fragment dungeons")
        # party
        self.make_sure_loaded(
            "./base/stockage/party.png",
            device,
            data["stockage"]["4"]["dms"],
            data["stockage"]["4"]["shell"],
            sleep_duration=0.5,
        )
        ...
        # start battle
        self.make_sure_loaded(
            "./base/stockage/select_battle.png",
            device,
            data["stockage"]["5"]["dms"],
            data["stockage"]["5"]["shell"],
            sleep_duration=0.5,
        )
        logging.info(device.serial + ": clicked start battle")
        # auto repeat
        self.make_sure_loaded(
            "./base/stockage/notice.png",
            device,
            data["stockage"]["6"]["dms"],
            data["stockage"]["6"]["shell"],
            sleep_duration=0.5,
        )
        logging.info(device.serial + ": clicked auto repeat")
        # select reward
        self.make_sure_loaded(
            "./base/stockage/fragment_select_reward.png",
            device,
            data["stockage"]["7"]["dms"],
            data["stockage"]["7"]["shell"],
            sleep_duration=0.5,
        )
        ...
        # click ok
        self.make_sure_loaded(
            "./base/stockage/loading_r.png",
            device,
            data["stockage"]["9"]["dms"],
            data["stockage"]["9"]["shell"],
            cutoff=10,
            loop=15,
            sleep_duration=0.5,
            second_shell=data["stockage"]["8-1"]["shell"] + str(r[0]) + " " + str(r[1]),
        )
        logging.info(
            device.serial + ": selected random reward and clicked ok to enter battle"
        )
        slp(5)
        # wait until finish
        self.make_sure_loaded(
            "./base/stockage/end.png",
            device,
            data["stockage"]["10"]["dms"],
            data["stockage"]["10"]["shell"],
            sleep_duration=15,
        )
        logging.info(device.serial + ": battle completed")
        # click exit
        self.make_sure_loaded(
            "./base/stockage/loading.png",
            device,
            data["stockage"]["11"]["dms"],
            data["stockage"]["11"]["shell"],
        )
        logging.info(device.serial + ": exited from fragment dungeons")

        # skill book dungeon
        self.make_sure_loaded(
            "./base/stockage/skill_book_d.png",
            device,
            data["stockage"]["12"]["dms"],
            data["stockage"]["12"]["shell"],
            sleep_duration=0.5,
        )
        ...
        self.make_sure_loaded(
            "./base/stockage/party.png",
            device,
            data["stockage"]["13"]["dms"],
            data["stockage"]["13"]["shell"],
            sleep_duration=0.5,
        )
        logging.info(device.serial + ": clicked to party setup")
        # check avalible team
        ...
        self.make_sure_loaded(
            "./base/stockage/select_battle.png",
            device,
            data["stockage"]["14"]["dms"],
            data["stockage"]["14"]["shell"],
            sleep_duration=0.5,
        )
        logging.info(device.serial + ": clicked start battle")
        # auto repeat
        self.make_sure_loaded(
            "./base/stockage/notice.png",
            device,
            data["stockage"]["15"]["dms"],
            data["stockage"]["15"]["shell"],
            sleep_duration=0.5,
        )
        logging.info(device.serial + ": clicked auto repeat")
        # select reward
        self.make_sure_loaded(
            "./base/stockage/exp_select_reward.png",
            device,
            data["stockage"]["16"]["dms"],
            data["stockage"]["16"]["shell"],
            sleep_duration=0.5,
        )
        logging.info(device.serial + ": clicked to select reward")
        ...
        self.make_sure_loaded(
            "./base/stockage/loading_r.png",
            device,
            data["stockage"]["19"]["dms"],
            data["stockage"]["19"]["shell"],
            cutoff=10,
            loop=15,
            sleep_duration=0.5,
            second_shell=data["stockage"]["18-1"]["shell"]
            + str(r[0])
            + " "
            + str(r[1]),
        )
        ...
        self.make_sure_loaded(
            "./base/stockage/end.png",
            device,
            data["stockage"]["20"]["dms"],
            data["stockage"]["20"]["shell"],
            sleep_duration=15,
        )
        logging.info(device.serial + ": battle completed")
        # click exit
        self.make_sure_loaded(
            "./base/stockage/loading.png",
            device,
            data["stockage"]["21"]["dms"],
            data["stockage"]["21"]["shell"],
        )
        logging.info(device.serial + ": exited from skill book dungeons")

        # exp dungeon
        self.make_sure_loaded(
            "./base/stockage/exp_d.png",
            device,
            data["stockage"]["22"]["dms"],
            data["stockage"]["22"]["shell"],
            sleep_duration=0.5,
        )
        ...
        self.make_sure_loaded(
            "./base/stockage/party.png",
            device,
            data["stockage"]["23"]["dms"],
            data["stockage"]["23"]["shell"],
            sleep_duration=0.5,
        )
        logging.info(device.serial + ": clicked to party setup")
        # check avalible team
        ...
        self.make_sure_loaded(
            "./base/stockage/select_battle.png",
            device,
            data["stockage"]["24"]["dms"],
            data["stockage"]["24"]["shell"],
            sleep_duration=0.5,
        )
        logging.info(device.serial + ": clicked start battle")
        # auto repeat
        self.make_sure_loaded(
            "./base/stockage/notice.png",
            device,
            data["stockage"]["25"]["dms"],
            data["stockage"]["25"]["shell"],
            sleep_duration=0.5,
        )
        logging.info(device.serial + ": clicked auto repeat")
        # click ok
        self.make_sure_loaded(
            "./base/stockage/loading_r.png",
            device,
            data["stockage"]["26"]["dms"],
            data["stockage"]["26"]["shell"],
            loop=10,
            sleep_duration=0.5,
        )
        ...
        self.make_sure_loaded(
            "./base/stockage/end.png",
            device,
            data["stockage"]["27"]["dms"],
            data["stockage"]["27"]["shell"],
            sleep_duration=15,
        )
        logging.info(device.serial + ": battle completed")
        # click exit
        self.make_sure_loaded(
            "./base/stockage/loading.png",
            device,
            data["stockage"]["28"]["dms"],
            data["stockage"]["28"]["shell"],
        )
        logging.info(device.serial + ": exited from exp dungeons")

        # gold dungeon
        self.make_sure_loaded(
            "./base/stockage/gold_d.png",
            device,
            data["stockage"]["29"]["dms"],
            data["stockage"]["29"]["shell"],
            sleep_duration=0.5,
        )
        logging.info(device.serial + ": clicked exp dungeons")
        # party
        self.make_sure_loaded(
            "./base/stockage/party.png",
            device,
            data["stockage"]["30"]["dms"],
            data["stockage"]["30"]["shell"],
            sleep_duration=0.5,
        )
        logging.info(device.serial + ": clicked to party setup")
        # check avalible team
        ...
        self.make_sure_loaded(
            "./base/stockage/select_battle.png",
            device,
            data["stockage"]["31"]["dms"],
            data["stockage"]["31"]["shell"],
            sleep_duration=0.5,
        )
        logging.info(device.serial + ": clicked start battle")
        # auto repeat
        self.make_sure_loaded(
            "./base/stockage/notice.png",
            device,
            data["stockage"]["32"]["dms"],
            data["stockage"]["32"]["shell"],
            sleep_duration=0.5,
        )
        logging.info(device.serial + ": clicked auto repeat")
        # click ok
        self.make_sure_loaded(
            "./base/stockage/loading_r.png",
            device,
            data["stockage"]["33"]["dms"],
            data["stockage"]["33"]["shell"],
            loop=10,
            sleep_duration=0.5,
        )
        logging.info(device.serial + ": clicked ok to enter battle")
        ...
        self.make_sure_loaded(
            "./base/stockage/end.png",
            device,
            data["stockage"]["34"]["dms"],
            data["stockage"]["34"]["shell"],
            sleep_duration=15,
        )
        logging.info(device.serial + ": battle completed")
        # click exit
        self.make_sure_loaded(
            "./base/stockage/loading.png",
            device,
            data["stockage"]["35"]["dms"],
            data["stockage"]["35"]["shell"],
        )
    ...
    def tower(self, device, position, data, lang):
        ...
        # click mission shortcut
        shortcut = self.make_sure_loaded(
            "./base/tower/tower.png",
            device,
            data["tower"]["1"]["dms"],
            data["tower"]["1"]["shell"] + position,
            loop=20,
            cutoff=20,
            sleep_duration=10,
        )
        ...
        logging.info(
            device.serial + ": clicked start battle and opened select battle board"
        )
    ...
    def wb(self, device, position, data):
        ...
        self.make_sure_loaded(
            ...
            cutoff=10,
            sleep_duration=0.5,
            second_shell=data["wb"]["5"]["second_shell"],
            loop=10,
        )
        logging.info(device.serial + ": clicked start battle")
        ...
        # click exit battle
        self.make_sure_loaded(
            "./base/wb/wb.png", device, data["wb"]["7"]["dms"], data["wb"]["7"]["shell"]
        )
        logging.info(device.serial + ": exited battle")
    ...
    def loh(self, device, data, lang):
        ...
        self.make_sure_loaded(
            ...
            data["loh"]["6"]["dms"],
            data["loh"]["6"]["shell"],
            second_img="./base/loh/previous_result.png",
            third_img="./base/loh/rewards.png",
            sleep_duration=10,
            loop=10,
            ck_special_shop=False,
        )
...
def run():
    ...
    if latest.json()["tag_name"] != this["version"]:
        ...
        def msg_box(this):
            answ = ctypes.windll.user32.MessageBoxW(
                0,
                text[1:].replace("\n", "\n\n") + "Do you want to remind this later?",
                f"kings-raid-daily new version ({latest.json()['tag_name']})",
                4,
            )
            if answ == 6:  # yes
                this["remind"] = True
                    ...
                    if re["max_devices"] == 1:
                                ...
                                if str(re_) + "/" == """b"player don't exist!"/""":
                                    ...
                                    while True:
                                        ...
                                        if devices != []:
                                            if (
                                                len(devices) == 1
                                                or launched_tem is not None
                                            ):
                                                for device in devices:
                                                    if str(device.serial).startswith(
                                                        "127"
                                                    ):
                                                        continue
                                                    if device.serial not in done:
                                                        thread = Thread(
                                                            target=Missions().run_execute,
                                                            args=(
                                                                device,
                                                                device_,
                                                            ),
                                                ...
                                                text = (
                                                    "'max_devices' set to 1 but 'adb devices' returns "
                                                    + str(len(devices))
                                                    + " devices, retrying..."
                                                )
                                                logging.info(text)
                                        ...
                                        if not_found_count >= 20:
                                            if value[0] is None:
                                                ...
                                                tem = value[0]
                                            re_ = run_(
                                                path + " launch --index " + str(tem),
                                                capture_output=True,
                                            ).stdout
                                            if (
                                                str(re_) + "/"
                                                == """b"player don't exist!"/"""
                                            ):
                                                text = (
                                                    "device with index "
                                                    + str(tem)
                                                    + " doesn't exist, unable to continue process"
                                                )
                                                logging.info(text)
                                                print(text)
                                                return
                        ...
                        while True:
                            ...
                            if running != 0:
                                        ...
                                        if int(thread_.name) not in done:
                                            ...
                                            while True:
                                                ...
                                                if elapsed_time > seconds:
                                                    break
                                                if (
                                                    thread_.is_alive() == False
                                                    or thread_._is_stopped == True
                                                ):
                                                    break
                                                slp(5)
                                ...
                                running, devices_dexist, dvices = launch_devices(
                                    running, devices_dexist
                                )
                                print("waiting 30 secs for fully boot up")
                                slp(30)
                                max_devices_count = 0
                                    ...
                                    if devices != []:
                                        if len(devices) == running:
                                            ...
                                            text = (
                                                "'max_devices' set to "
                                                + str(re["max_devices"])
                                                + " but 'adb devices' returns "
                                                + str(len(devices))
                                                + " devices, retrying for "
                                                + str(50 - max_devices_count)
                                                + " times..."
                                            )
                                            logging.info(text)
                                            print(text)
                                            ...
                                            if max_devices_count >= 50:
                                                print(
                                                    "not enough device(s) to continue running, quitall and retrying..."
                                                )
                                                run_(path + " quitall")
                                                slp(5)
                                                running = running_
                                                ...
                                                print(
                                                    "waiting 30 secs for fully boot up"
                                                )
                                                slp(30)
                                                max_devices_count = 0
                                            continue
